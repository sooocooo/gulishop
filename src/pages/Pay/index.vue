    ...
    async pay() {
      ...
      try {
        ...
        this.$alert(`<img src="${imgUrl}" / >`, "请支付", {
          ...
          center: true, //是否居中布局
          //4. 点击按钮之后的处理以及和第三部产生联系
          beforeClose: (action, instance, done) => {
            if (action === "confirm") {
              // if (this.status !== 200) {
              //    this.$message.wraining('没支付!')
              // }
              //测试
        ...
        //3. 弹出消息的同时循环的给后台发送请求 ,获取该订单的支付状态数据
        //根据返回来的支付状态数据去决定要不要跳转到支付页面
        if (!this.timer) {
        ...
        this.$message.error("二维码失败" + error.message);
      }
    },
